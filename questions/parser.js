...
async function main() {
  ...
  await ConstQuestion.deleteMany({}, err => {
    console.log(err || "Successfully deleted all entries from database");
  })
  ...
  console.log(`Finished adding all (${questionCount}) entries to database`)

  const endTime = performance.now()
  console.log(`Parser finished! (${Math.floor(endTime - inTime + .5)}ms)`);
}
